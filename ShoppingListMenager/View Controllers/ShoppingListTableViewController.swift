...
class ShoppingListTableViewController: UITableViewController {
    ...
    var shoppingListArray = [
        ShoppingList(user: User(login: "test", password: "sdfsdf", email: "fsdfsd"), shoppingItemArray: Array<ShoppingItem>()),
        ShoppingList(user: User(login: "test", password: "sdfsdf", email: "fsdfsd"), shoppingItemArray: Array<ShoppingItem>()),
        ShoppingList(user: User(login: "test", password: "sdfsdf", email: "fsdfsd"), shoppingItemArray: Array<ShoppingItem>()),
                             ]
    ...
    override func viewWillAppear(_ animated: Bool) {
        if let realmShoppingLists = realmController.getShoppingList(){
            var shoppingList = Array<ShoppingList>()
            for realmShoppingList in realmShoppingLists{
                shoppingList.append(ShoppingList(realmShoppingList: realmShoppingList))
            }
        }        
        tableView.reloadData()
    }
//    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
//        print("\(shoppingListArray[indexPath.row].name)")
//    }
